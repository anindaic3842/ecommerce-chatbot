...
const handleFormSubmission = async (req, res) => {
  ...
  try {
    ...
    db.collection("contactus").insertOne(formData)
      // .then((result) => {
      //   logger.log(`New contact inserted with ID: ${result.insertedId}`);
      //   res.json({
      //     success: true,
      ...
      // })
      // .catch((error) => {
      //   logger.error("Error inserting data into MongoDB:", error);
      //   res
      //     .status(500)
      //     .json({
      //       success: false,
...
const checkEmailfromCustomer = async (req, res) => {
  if (req.body == null || req.body.sessionInfo == null || req.body.sessionInfo.parameters == null || req.body.sessionInfo.parameters.email == null) {
    logger.error(
